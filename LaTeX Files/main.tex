...
Additionally, we describe the big-step semantics for Wrangell's operations in Figure \ref{bigStep}.
...
We planned to use the IO (`input/output') monad to handle user input to Wrangell. The IO monad would be useful in \emph{monad transformers}, wherein we would adapt data types in existing monads to also include input/output functionality. For this purpose, monad transformations would allow us to combine the behaviors of the IO monad with that of, say, error-handling monads into one data type. 

The IOThrowsError data type, for example, was inspired by an example we found in a Haskell tutorial \cite{writeScheme48}. Construction of this data type in Figure \ref{IOThrowsError} uses the monad transformer \emph{ExceptT}. ExceptT incorporates the functionality of the Either monad and facilitates application of error handling to Wrangell errors (WError) in the IO monad. Observe Wrangell construction and sample usage of IOThrowsError in Figure \ref{IOThrowsError}). In checkAllUnique which checks if the column labels provided to Wrangell are unique, we see the use of ThrowError (which exists in the Either error-handling monad) \emph{and} () which exists in the IO monad???... 
...
\begin{figure}
\caption{WANT TO GET RID OF THIS. RUNREPL NOT THAT IMPORTANT. IO monad used in runRepl, a function which ...()}
\label{runReplIO}
\begin{lstlisting}
-- main.hs --
runRepl :: IO ()
